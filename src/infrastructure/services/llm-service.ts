 ...
 *
 * Provides LLM-powered capabilities for task clarification, next action suggestions and prioritization
 */
...
export interface LLMService {
  ...
  getEisenhowerAssessment(
    taskDescription: string,
  ): Promise<EisenhowerResponse | null>;
}
...
export class LangChainLLMService implements LLMService {
  ...
  async getClarification(
    ...
    console.log(
      `[LLM Service] Getting clarification for: '${taskDescription}'`,
    );
    ...
    try {
      ...
      console.error(
        "[LLM Service - Error] LangChain clarification call failed:",
        error,
      );
      return {
        actionable: null,
  ...
  async getNextActionSuggestion(
    projectName: string,
    projectOutcome: string | null,
  ): Promise<string> {
    console.log(
      `[LLM Service] Getting next action for: Project '${projectName}'`,
    );
    ...
    try {
      ...
      console.error(
        "[LLM Service - Error] LangChain next action call failed:",
        error,
      );
  ...
  async getEisenhowerAssessment(
    taskDescription: string,
  ): Promise<EisenhowerResponse | null> {
    console.log(
      `[LLM Service] Getting Eisenhower assessment for: '${taskDescription}'`,
    );
    ...
    try {
      ...
      console.error(
        "[LLM Service - Error] LangChain Eisenhower call failed:",
        error,
      );
      return {
        urgent: null,
