...
export class TodoistLoader {
  ...
  public async loadRawData(): Promise<TodoistExportData> {
    ...
    const validation = validateFile(
      this.filePath,
      "todoist_denormalized_schema",
    );

    if (!validation.valid) {
      throw new Error(
        `Todoist export validation failed: ${validation.errors.join(", ")}`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Error loading Todoist export data: ${(err as Error).message}`,
      );
    }
  }
  ...
  private mapTasks(
    ...
    for (const todoistTask of todoistTasks) {
      if (
        todoistTask.parent_id &&
        taskMap.has(todoistTask.parent_id) &&
        taskMap.has(todoistTask.id)
      ) {
  ...
  private mapProjects(todoistProjects: TodoistProject[]): Project[] {
    ...
    for (const todoistProject of todoistProjects) {
      if (
        todoistProject.parent_id &&
        projectMap.has(todoistProject.parent_id) &&
        projectMap.has(todoistProject.id)
      ) {
