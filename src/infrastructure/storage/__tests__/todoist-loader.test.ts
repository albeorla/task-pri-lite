...
describe("TodoistLoader", () => {
  ...
  beforeEach(() => {
    ...
    (fs.promises.readFile as jest.Mock).mockResolvedValue(
      JSON.stringify(mockTodoistData),
    );
  });
  ...
  it("should throw an error if validation fails", async () => {
    ...
    await expect(loader.loadRawData()).rejects.toThrow(
      "Todoist export validation failed: Test error",
    );
  });
  ...
  it("should throw an error if file reading fails", async () => {
    ...
    await expect(loader.loadRawData()).rejects.toThrow(
      "Error loading Todoist export data: File read error",
    );
  });
