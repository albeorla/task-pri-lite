...
export class TaskManager {
  ...
  async runWorkflow(): Promise<void> {
    ...
    const inboxTasks = Array.from(this.tasks.values()).filter(
      (t) => t.status === TaskStatus.INBOX,
    );
    ...
    for (const task of inboxTasks) {
      try {
        ...
        console.error(
          `[Error] Failed processing task '${task.description}'. Error:`,
          error,
        );
      }
    }
    ...
    console.log(
      `-- Processing Step Complete (${processedCount} tasks processed) --`,
    );
    ...
    for (const task of this.tasks.values()) {
      ...
      if (
        ...
        try {
          ...
          console.error(
            `[Error] Failed prioritizing task '${task.description}'. Error:`,
            error,
          );
        }
      }
    ...
    console.log(
      `-- Prioritization Step Complete (${prioritizedCount} tasks assessed) --`,
    );
    console.log("\n=== Workflow Complete ===");
  }
   ...
   */
  printSummary(
    groupBy: "status" | "project" | "context" | "priority" = "status",
  ): void {
    ...
    if (this.projects.size === 0) {
      ...
      for (const project of sortedProjects) {
        ...
        console.log(
          `    Next Action: ${nextAction ? nextAction.description : "*** NONE DEFINED ***"}`,
        );
      }
    }
