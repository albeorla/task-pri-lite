...
export class GTDClarificationProcessor implements TaskProcessor {
  ...
  async process(task: Task, projects: Map<string, Project>): Promise<void> {
    ...
    if (this.llmService) {
      const clarification = await this.llmService.getClarification(
        task.description,
      );
      if (clarification) {
        ...
        console.log(
          `  LLM Analysis: Actionable=${task.isActionable}, Project=${isProject}, Outcome=${outcome}, Rationale=${rationale}`,
        );

        // Handle LLM errors or null values
        if (task.isActionable === null) {
          console.log(
            "  LLM clarification inconclusive, proceeding with manual input.",
          );
        ...
        if (!task.isActionable) {
          if (task.status === TaskStatus.INBOX) {
            ...
            task.status =
              rationaleText.includes("someday") ||
              rationaleText.includes("maybe")
  ...
  private async handleProjectTask(
    ...
    console.log(
      `  Task '${task.description}' added to project '${project.name}'.`,
    );
  ...
  private async defineNextAction(
    project: Project,
    taskIfOnlyOne?: Task,
  ): Promise<void> {
    if (project.getNextAction()) {
      console.log(
        `  Project '${project.name}' already has a Next Action defined.`,
      );
      return;
    }
    ...
    if (this.llmService) {
      try {
        suggestion = await this.llmService.getNextActionSuggestion(
          project.name,
          project.outcome,
        );
        ...
        console.error(
          `    [Error] Failed to get LLM suggestion for next action:`,
          error,
        );
        suggestion = "";
      }
    ...
    if (
      taskIfOnlyOne &&
      userNextAction.trim().toLowerCase() ===
        taskIfOnlyOne.description.trim().toLowerCase()
    ) {
      taskToMark = taskIfOnlyOne;
      console.log(
        `    Marking existing task '${taskToMark.description}' as Next Action.`,
      );
    } else {
      ...
      if (existingTask) {
        ...
        console.log(
          `    Created new Next Action task: '${taskToMark.description}'`,
        );
      }
    }
  ...
  private async assignContext(task: Task): Promise<void> {
    ...
    console.log(
      "\n    Available Contexts:",
      Array.from(this.contexts).sort().join(", "),
    );
    ...
    if (
      ...
      task.description.toLowerCase().includes("shop")
    ) {
