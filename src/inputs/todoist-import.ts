...
export class TodoistImporter {
  ...
  private sectionMap: Map<string, { name: string, project: string }> = new Map();
          ...
          project.tasks.forEach((task: TodoistTask) => {
            ...
            if (task.sub_tasks && Array.isArray(task.sub_tasks)) {
              task.sub_tasks.forEach((subtask: TodoistTask) => {
                taskItems.push(this.convertToTaskItem(subtask, project.name, task.content));
              });
            }
          });
        }
          ...
          project.sections.forEach((section: TodoistSection) => {
            if (section.tasks && Array.isArray(section.tasks)) {
              section.tasks.forEach((task: TodoistTask) => {
                taskItems.push(this.convertToTaskItem(task, project.name, undefined, section.name));
                
                // Process subtasks if any
                if (task.sub_tasks && Array.isArray(task.sub_tasks)) {
                  task.sub_tasks.forEach((subtask: TodoistTask) => {
                    taskItems.push(this.convertToTaskItem(subtask, project.name, task.content, section.name));
                  });
                }
              });
            }
   ...
   */
  private processProjectTasks(project: TodoistProject, items: (TextInputItem | ManualTaskInputItem)[]): void {
    // Process main tasks in project
    if (project.tasks && Array.isArray(project.tasks)) {
      project.tasks.forEach((task: TodoistTask) => {
        ...
        if (task.sub_tasks && Array.isArray(task.sub_tasks)) {
          task.sub_tasks.forEach((subtask: TodoistTask) => {
            items.push(this.convertToInputItem(subtask, project.name, task.content));
          });
        }
      });
    }
    ...
    if (project.sections && Array.isArray(project.sections)) {
      project.sections.forEach((section: TodoistSection) => {
        if (section.tasks && Array.isArray(section.tasks)) {
          section.tasks.forEach((task: TodoistTask) => {
            items.push(this.convertToInputItem(task, project.name, undefined, section.name));
            
            // Process subtasks if any
            if (task.sub_tasks && Array.isArray(task.sub_tasks)) {
              task.sub_tasks.forEach((subtask: TodoistTask) => {
                items.push(this.convertToInputItem(subtask, project.name, task.content, section.name));
              });
            }
          });
        }
